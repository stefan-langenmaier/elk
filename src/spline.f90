...
t5=x2**2
t6=x3**2
cf(1,1)=t0*(t2*(x3*t5-x2*t6)+t3*(x1*t6-x3*t4)+t1*(x2*t4-x1*t5))
cf(2,1)=t0*(t2*(t6-t5)+t3*(t4-t6)+t1*(t5-t4))
cf(3,1)=t0*(t2*(x2-x3)+t3*(x3-x1)+t1*(x1-x2))
cf(1,2)=cf(1,1)+2.d0*cf(2,1)*x1+3.d0*cf(3,1)*t4
cf(2,2)=cf(2,1)+3.d0*cf(3,1)*x1
cf(3,2)=cf(3,1)
if (n.eq.4) then
  cf(1,3)=cf(1,1)+2.d0*cf(2,1)*x2+3.d0*cf(3,1)*t5
  cf(2,3)=cf(2,1)+3.d0*cf(3,1)*x2
  cf(3,3)=cf(3,1)
  cf(1,4)=cf(1,1)+2.d0*cf(2,1)*x3+3.d0*cf(3,1)*t6
  cf(2,4)=cf(2,1)+3.d0*cf(3,1)*x3
  cf(3,4)=cf(3,1)
  return
end if
do i=3,n-2
  ...
  t5=x2**2
  t6=x3**2
  cf(1,i)=t0*(t2*(x3*t5-x2*t6)+t3*(x1*t6-x3*t4)+t1*(x2*t4-x1*t5))
  cf(2,i)=t0*(t2*(t6-t5)+t3*(t4-t6)+t1*(t5-t4))
  cf(3,i)=t0*(t2*(x2-x3)+t3*(x3-x1)+t1*(x1-x2))
end do
cf(1,n-1)=cf(1,n-2)+2.d0*cf(2,n-2)*x2+3.d0*cf(3,n-2)*t5
cf(2,n-1)=cf(2,n-2)+3.d0*cf(3,n-2)*x2
cf(3,n-1)=cf(3,n-2)
cf(1,n)=cf(1,n-2)+2.d0*cf(2,n-2)*x3+3.d0*cf(3,n-2)*t6
cf(2,n)=cf(2,n-2)+3.d0*cf(3,n-2)*x3
cf(3,n)=cf(3,n-2)
return
end subroutine
!EOC
